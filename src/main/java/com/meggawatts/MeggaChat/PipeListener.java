...
public class PipeListener implements Listener {
    ...
    public void phialLight(PlayerInteractEvent event) {
		if((event.getPlayer().hasPermission("meggachat.phial"))
				&& (event.hasItem())
			...
			else if(!(event.getPlayer().hasPotionEffect(PotionEffectType.NIGHT_VISION))
	        		&& (((event.getAction().equals(Action.RIGHT_CLICK_AIR)) 
	        		|| (event.getAction().equals(Action.RIGHT_CLICK_BLOCK)))) 
	        		){
				double distance = 0.7D;
    ...
    public void oneRing(PlayerInteractEvent event) {
		if((event.getPlayer().hasPermission("meggachat.ring"))
				&& (event.hasItem())
			...
			if((event.getPlayer().hasPotionEffect(PotionEffectType.INVISIBILITY))
				...
				if(sfx){
		            ...
		            World w = p.getWorld();
		            w.playSound(p.getLocation(), Sound.ENDERDRAGON_GROWL, 1, 0);
		            }
			}
				...
				if(sfx){
		            Player p = (Player) event.getPlayer();
		            World w = p.getWorld();
		            w.playSound(p.getLocation(), Sound.WITHER_DEATH, 1, 1);
		            w.playSound(p.getLocation(), Sound.ENDERMAN_STARE, 1, 0);
		            }
			}
		}
    }
